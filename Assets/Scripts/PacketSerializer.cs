...
public class PacketSerializer : Network.Serializer
{
    ...
    {
        GameEvent = 2,                      // generic packet about gameplay events (currently thinking about scoring). Sent both directions (request/confirm), and sent "along" by server to other clients to confirm.
    ...
    public void WriteGameEventPacket(Network.WriteStream stream, GameEvent eventType, ushort senderId, ushort targetId, ushort[] perClientData)
    {
        byte packetType = (byte)PacketType.GameEvent;
        byte gameEventType = (byte)eventType;

        write_bits(stream, packetType, 8);
        write_bits(stream, gameEventType, 8);

        //write sender (player that scored) & target (owner of goal)
        write_bits(stream, senderId, 16);
        write_bits(stream, targetId, 16);

        for (int i = 0; i < Constants.MaxClients; ++i)
        {
            write_bits(stream, perClientData[i], 16);
        }
    }

    public void ReadGameEventPacket(Network.ReadStream stream, ref GameEvent eventType, ref ushort senderId, ref ushort targetId, ref ushort[] perClientData)
    {
        byte packetType = 0;
        byte gameEventType = 0;

        read_bits(stream, out packetType, 8);

        Debug.Assert(packetType == (byte)PacketType.GameEvent);

        read_bits(stream, out gameEventType, 8);

        switch( (GameEvent)gameEventType) {
            case GameEvent.SCORE:
                read_bits(stream, out senderId, 16);
                read_bits(stream, out targetId, 16);

                for (int i = 0; i < Constants.MaxClients; ++i)
                {
                    read_bits(stream, out perClientData[i], 16);
                }
                break;
        }
    }
